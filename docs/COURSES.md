...
- 从小函数开始，逐步增加参数
- 最后一课是自由创作热身（L5-20），L6 才是阶段总结
- 强调"写一次，用很多次"的便利

### 3.6 L6 — 综合挑战

**主题**：L1~L5 知识综合演练，不引入新概念

**编写要点**：
- 每课是结构化项目，starterCode 给 60% 骨架，孩子补全 `# TODO`
- checks 4~6 条：结构（contains）+ 输出（output_contains）+ no_error
- 含 input() 的课：结构 check 为主，output 为辅
- 难度略高于普通课，低于竞技场（无 max_lines 限制）
- 线性解锁：l6_21 → l6_22 → l6_23 → l6_24，前置 l5_20
